...
> - **Status:** DRAFT
> - **Service ID:** `shared-wf-svc`
> - **Suite:** `shared`
> - **Domain:** `wf`
> - **Base Package:** `io.openleap.shared.wf`
> - **API Path:** `/api/shared/wf/v1`
> - **Port:** 8415
> - **DB Schema:** `shared_workflow`
...
This document specifies the **Workflow Service** (`shared-wf-svc`) as a T2 Shared Enterprise Business capability: a suite-agnostic rule-based automation engine consumed by any T3 suite.
...
- OpenAPI: `contracts/http/shared/wf/openapi.yaml`
- Event Schemas: `contracts/events/shared/wf/*.schema.json`
...
| Base Package | `io.openleap.shared.wf` |
| API Base Path | `/api/shared/wf/v1` |
| Port | 8415 |
| Database Schema | `shared_workflow` |
| Tier | T2 — Shared Enterprise Business |
| Repository | `openleap-io/io.openleap.shared.wf` |
...
**Base Path:** `/api/shared/wf/v1`
**Authentication:** OAuth2 / JWT (Bearer). **Authorization:** scopes `shared.wf:read`, `shared.wf:write`, `shared.wf:admin`.

Endpoint table above is the canonical summary. Full payloads + request/response schemas in `contracts/http/shared/wf/openapi.yaml`.
...
| Event | Routing Key |
|-------|------------|
| WorkflowTriggered | `shared.wf.workflow.triggered` |
| WorkflowCompleted | `shared.wf.workflow.completed` |
| WorkflowFailed | `shared.wf.workflow.failed` |
| AssignmentPerformed | `shared.wf.assignment.performed` |
| EscalationFired | `shared.wf.escalation.fired` |
...
For 60 days after promotion, `shared-wf-svc` SHALL also publish outbound events under legacy `crm.wf.*` keys.

---

## 8. Data Model

**Storage:** PostgreSQL 16, Schema: `shared_workflow`. Multi-tenant RLS on `tenant_id`.
...
|-----------|-------------|
| `F-SHARED-WF-001` | Workflow Builder |
| `F-SHARED-WF-002` | Workflow Template Library |
| `F-SHARED-WF-003` | Workflow Monitor & Logs |
| `F-SHARED-WF-004` | Assignment & Escalation Rules |
...
- Extension events `shared.wf.ext.pre-execute`, `shared.wf.ext.post-execute` published so tenant-specific logic can observe every execution.
...
- Routing keys: new `shared.wf.*`; old `crm.wf.*` re-emitted 60 days.
- API path `/api/shared/wf/v1`; `/api/crm/wf/v1` returns HTTP 308 during grace period.
- Schema `crm_workflow` renamed to `shared_workflow` via Flyway.
- Port stays 8415.
...
See `T2_SharedBusiness/_t2_suite.md` SS1.
