# F-TKS-100-01 — Create Ticket (agent form)
...
> **Companion UVL:** `F-TKS-100-01.uvl`
> **Companion AUI:** `contracts/aui/F-TKS-100-01.aui.yaml`

---

## SS0 — Identity & Purpose

**Purpose:** Agent-facing form to create a new ticket from scratch. Collects title, description, reporter, category, priority, attachments. Validates against SLA policy and (optionally) calls KB suggest-on-create.
...
Canonical view-model composed by BFF; see `contracts/aui/F-TKS-100-01.aui.yaml` for the AUI (abstract UI) contract and `../domain-specs/*.md` §6 for the underlying REST calls.
