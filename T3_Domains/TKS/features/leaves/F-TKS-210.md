...
> **Companion UVL:** `F-TKS-210.uvl`
> **Companion AUI:** `contracts/aui/F-TKS-210.aui.yaml`

---

## SS0 — Identity & Purpose

**Purpose:** Polls / receives inbound email via `tech.email`. Resolves sender to `shared.bp` party, deduplicates by provider message-id, classifies, and produces `tks.ch.conversation.message-received`.
...
Canonical view-model composed by BFF; see `contracts/aui/F-TKS-210.aui.yaml` for the AUI (abstract UI) contract and `../domain-specs/*.md` §6 for the underlying REST calls.
