...
> **Companion files:** `F-PS-006-01.uvl` (§9), `F-PS-006-01.aui.yaml` (§6, future), `F-PS-006-01.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-006-01`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-006` — see `F-PS-006.md`
> - **Companion UVL:** `F-PS-006-01.uvl`
> - **Companion AUI:** `F-PS-006-01.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-006-01.attrs.md` *(if needed)*
...
This feature lets a Portfolio Manager view a cross-project dashboard showing status, budget health, resource demand, and lifecycle progress for all projects in a portfolio, so that strategic oversight is maintained.
...
- Main navigation → Portfolios → select portfolio

### 0.4 Exit Points

- Click project → Project detail
- Click 'Gate Review' → Gate Review (F-PS-006-04)
...
**When:** PFM opens Portfolio Dashboard. System shows: portfolio summary (active/completed/cancelled project counts), project status matrix (projects × lifecycle phases with current phase highlighted), budget health summary (on budget / at risk / overspent per project), resource demand vs. capacity chart, and a project list sortable by priority, status, and health score.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Drill down to project

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PFM clicks a project in the dashboard. System navigates to the project detail view.
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: Filter by program

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PFM filters the dashboard to a specific program. System recalculates all metrics for only projects in that program.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Portfolio Dashboard
    UI->>SVC: GET /api/ps/prt/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Portfolio Dashboard screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Portfolio Dashboard
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-006-01.aui.yaml`.
...
- **AC-F-PS-006-01-01:** Given the preconditions for 'Happy path: view dashboard', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-006-01-02:** Given the preconditions for 'Drill down to project', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-006-01-03:** Given the preconditions for 'Filter by program', when the user performs the described action, then the system behaves as specified in Scenario 3.

- **AC-F-PS-006-01-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-006-01-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-006-01-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-006-01.uvl)* | | |
