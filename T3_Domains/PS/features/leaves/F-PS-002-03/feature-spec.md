...
> **Companion files:** `F-PS-002-03.uvl` (§9), `F-PS-002-03.aui.yaml` (§6, future), `F-PS-002-03.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-002-03`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-002` — see `F-PS-002.md`
> - **Companion UVL:** `F-PS-002-03.uvl`
> - **Companion AUI:** `F-PS-002-03.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-002-03.attrs.md` *(if needed)*
...
This feature lets team members visualize work packages as cards on a columnar Kanban board, drag cards between columns to update status, and see WIP limits, so that work flow is managed visually.
...
- Main navigation → Boards

### 0.4 Exit Points

- Click card → Work Package Detail (F-PS-001-05)
- Configure board → Board Settings
...
#### Scenario 1: Happy path: view board

**Given:** The user has appropriate access permissions and the system is operational.
**When:** User opens the project board. System shows the default board with columns configured by status (e.g., New → In Progress → Review → Done). Each column shows WP cards with: code, subject, assignee avatar, story points badge, priority indicator.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Drag to change status

**Given:** The user has appropriate access permissions and the system is operational.
**When:** User drags a WP card from 'In Progress' to 'Review'. System updates the WP status in ps-prj-svc via API call and publishes ps.prj.workpackage.updated. The card animates to its new column.
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: WIP limit warning

**Given:** The user has appropriate access permissions and the system is operational.
**When:** A column has a WIP limit of 3 and already has 3 cards. User drags a 4th card in. System shows a warning (BR-AGL-007) — the move is allowed but the column header turns red to indicate limit exceeded.
**Then:** The expected outcome is achieved as described above.


#### Scenario 4: Filter board

**Given:** The user has appropriate access permissions and the system is operational.
**When:** User filters the board by assignee, sprint, or WP type. System shows only matching cards.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Kanban Board
    UI->>SVC: GET /api/ps/agl/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Kanban Board screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Kanban Board
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-002-03.aui.yaml`.
...
- **AC-F-PS-002-03-01:** Given the preconditions for 'Happy path: view board', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-002-03-02:** Given the preconditions for 'Drag to change status', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-002-03-03:** Given the preconditions for 'WIP limit warning', when the user performs the described action, then the system behaves as specified in Scenario 3.
- **AC-F-PS-002-03-04:** Given the preconditions for 'Filter board', when the user performs the described action, then the system behaves as specified in Scenario 4.

- **AC-F-PS-002-03-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-002-03-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-002-03-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-002-03.uvl)* | | |
