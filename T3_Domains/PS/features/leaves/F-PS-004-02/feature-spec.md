...
> **Companion files:** `F-PS-004-02.uvl` (§9), `F-PS-004-02.aui.yaml` (§6, future), `F-PS-004-02.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-004-02`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-004` — see `F-PS-004.md`
> - **Companion UVL:** `F-PS-004-02.uvl`
> - **Companion AUI:** `F-PS-004-02.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-004-02.attrs.md` *(if needed)*
...
This feature lets a team member record non-labor costs (materials, travel, licenses, etc.) against a work package, so that all project costs beyond labor are tracked.
...
- Main navigation → Cost Entry
- Work Package Detail → 'Log Cost' button

### 0.4 Exit Points

- Entry saved → cost list
- Submit for approval
...
#### Scenario 1: Happy path: record cost

**Given:** The user has appropriate access permissions and the system is operational.
**When:** User opens Cost Entry form. System shows: project selector, work package selector, cost type dropdown (loaded from ps-bud-svc), date, quantity, unit rate (auto-filled from cost type rate schedule, editable), currency. User enters quantity (e.g., 500 km). System calculates totalAmount = quantity × unitRate. User saves.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Rate auto-fill

**Given:** The user has appropriate access permissions and the system is operational.
**When:** User selects cost type 'Travel'. System looks up the effective rate for today's date from the rate schedule and fills in the unit rate. User can override if needed.
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: Validation: quantity zero

**Given:** The user has appropriate access permissions and the system is operational.
**When:** User enters quantity 0. System rejects with error BR-TIM-006.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Cost Entry
    UI->>SVC: GET /api/ps/tim/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Cost Entry screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Cost Entry
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-004-02.aui.yaml`.
...
- **AC-F-PS-004-02-01:** Given the preconditions for 'Happy path: record cost', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-004-02-02:** Given the preconditions for 'Rate auto-fill', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-004-02-03:** Given the preconditions for 'Validation: quantity zero', when the user performs the described action, then the system behaves as specified in Scenario 3.

- **AC-F-PS-004-02-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-004-02-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-004-02-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-004-02.uvl)* | | |
