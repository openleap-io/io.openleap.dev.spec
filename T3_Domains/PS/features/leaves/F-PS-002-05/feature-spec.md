...
> **Companion files:** `F-PS-002-05.uvl` (§9), `F-PS-002-05.aui.yaml` (§6, future), `F-PS-002-05.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-002-05`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-002` — see `F-PS-002.md`
> - **Companion UVL:** `F-PS-002-05.uvl`
> - **Companion AUI:** `F-PS-002-05.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-002-05.attrs.md` *(if needed)*
...
This feature lets a Scrum Master view the team's velocity (story points completed per sprint) as a bar chart over past sprints, so that sprint capacity can be estimated for future planning.
...
- Agile Dashboard → 'Velocity' widget
- Sprint Planning → velocity reference

### 0.4 Exit Points

- Click bar → Sprint detail
...
#### Scenario 1: Happy path: view velocity

**Given:** The user has appropriate access permissions and the system is operational.
**When:** SM opens the Velocity tab. System shows a bar chart: X-axis = sprint names (chronological), Y-axis = story points. Each bar has two segments: committed (lighter) and completed (darker). A dashed line shows the rolling average velocity. SM can see if the team's capacity is stable, improving, or declining.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Forecast capacity

**Given:** The user has appropriate access permissions and the system is operational.
**When:** SM creates a new sprint. System suggests a commitment range based on the rolling average velocity (e.g., 'Based on your average velocity of 34 points, consider committing 30-38 points').
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Velocity Tracking
    UI->>SVC: GET /api/ps/agl/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Velocity Tracking screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Velocity Tracking
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-002-05.aui.yaml`.
...
- **AC-F-PS-002-05-01:** Given the preconditions for 'Happy path: view velocity', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-002-05-02:** Given the preconditions for 'Forecast capacity', when the user performs the described action, then the system behaves as specified in Scenario 2.

- **AC-F-PS-002-05-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-002-05-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-002-05-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-002-05.uvl)* | | |
