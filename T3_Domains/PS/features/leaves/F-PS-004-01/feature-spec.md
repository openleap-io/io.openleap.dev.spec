...
> **Companion files:** `F-PS-004-01.uvl` (§9), `F-PS-004-01.aui.yaml` (§6, future), `F-PS-004-01.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-004-01`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-004` — see `F-PS-004.md`
> - **Companion UVL:** `F-PS-004-01.uvl`
> - **Companion AUI:** `F-PS-004-01.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-004-01.attrs.md` *(if needed)*
...
This feature lets a team member record hours worked against a work package on a specific date, so that project time is captured for cost tracking and progress measurement.
...
- Main navigation → Time Entry
- Work Package Detail → 'Log Time' button
- My Time → 'Add Entry'

### 0.4 Exit Points

- Entry saved → Time list or weekly view
...
#### Scenario 1: Happy path: log time

**Given:** The user has appropriate access permissions and the system is operational.
**When:** User opens Time Entry form. System shows: project selector (projects user has access to), work package selector (filtered by project, only active WPs per BR-TIM-003), date picker (defaults to today, cannot be future per BR-TIM-002), hours field, activity type dropdown (optional), comment field. User fills in fields and clicks Save. System creates time booking in DRAFT status.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Weekly timesheet view

**Given:** The user has appropriate access permissions and the system is operational.
**When:** User opens the weekly timesheet. System shows a grid: rows = work packages the user has recently booked to, columns = days of the week, cells = hours. User can enter hours directly in cells. Total hours per day and per WP are calculated automatically.
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: Validation: hours exceed 24

**Given:** The user has appropriate access permissions and the system is operational.
**When:** User enters 25 hours for a single day. System rejects with error BR-TIM-001.
**Then:** The expected outcome is achieved as described above.


#### Scenario 4: Validation: future date

**Given:** The user has appropriate access permissions and the system is operational.
**When:** User tries to log time for tomorrow. System rejects with error BR-TIM-002.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Time Booking Entry
    UI->>SVC: GET /api/ps/tim/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Time Booking Entry screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Time Booking Entry
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-004-01.aui.yaml`.
...
- **AC-F-PS-004-01-01:** Given the preconditions for 'Happy path: log time', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-004-01-02:** Given the preconditions for 'Weekly timesheet view', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-004-01-03:** Given the preconditions for 'Validation: hours exceed 24', when the user performs the described action, then the system behaves as specified in Scenario 3.
- **AC-F-PS-004-01-04:** Given the preconditions for 'Validation: future date', when the user performs the described action, then the system behaves as specified in Scenario 4.

- **AC-F-PS-004-01-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-004-01-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-004-01-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-004-01.uvl)* | | |
