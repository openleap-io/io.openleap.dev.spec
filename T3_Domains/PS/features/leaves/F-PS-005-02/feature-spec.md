...
> **Companion files:** `F-PS-005-02.uvl` (§9), `F-PS-005-02.aui.yaml` (§6, future), `F-PS-005-02.attrs.md` (§9, if needed)
> **Referenced by:** Product Spec SS17 (Feature Selection), Suite Feature Catalog SS6
> **References:** Domain/Service Spec `ps_res-spec.md` (SS5 backend deps), Suite Spec `_ps_suite.md`
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-005-02`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-005` — see `F-PS-005.md`
> - **Companion UVL:** `F-PS-005-02.uvl`
> - **Companion AUI:** `F-PS-005-02.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-005-02.attrs.md` *(if needed)*
...
This feature lets a PM or Resource Manager create and manage staffing slots on a project, defining demand for specific personas over time periods, so that resource needs are formally documented.
...
- Slot created → staffing list
- Assign resource → resource selection (queries ops-res-svc)
...
#### Scenario 1: Happy path: create staffing slot

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM opens Staffing tab for a project. PM clicks 'Add Staffing Demand'. System shows form: persona (dropdown from catalog), start date, end date, FTE percentage (e.g., 0.5 = 50%), comment. PM saves. Slot is created in OPEN status.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Fill slot with named resource

**Given:** The user has appropriate access permissions and the system is operational.
**When:** RM selects an open slot and clicks 'Assign Resource'. System queries ops-res-svc for available resources matching the persona's skills. RM selects a person. System updates slot to FILLED status with assignedResourceId and publishes ps.res.slot.filled event.
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: Release resource

**Given:** The user has appropriate access permissions and the system is operational.
**When:** RM releases a filled slot. System clears the assignment, moves slot to RELEASED status, and publishes ps.res.slot.released event.
**Then:** The expected outcome is achieved as described above.


#### Scenario 4: Invalid FTE

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM enters FTE of 3.0 (>2.0). System rejects (BR-RES-002) with error.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    participant SVC as ps-res-svc
    participant EVT as Event Bus

    U->>UI: Navigate to Staffing Slot Management
    UI->>SVC: GET /api/ps/res/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Staffing Slot Management screen
    U->>UI: Perform action
    UI->>SVC: POST/PATCH /api/ps/res/v1/...
...
Fields are defined by the aggregate attributes in `ps_res-spec.md`. Key fields for this feature:
...
|---------|------|----------|--------|------------|--------------|
| `ps-res-svc` | T3 | `/api/ps/res/v1/...` | GET/POST/PATCH | Read: No, Write: Yes | Retry with backoff; degrade to cached data for reads |
...
```jsonc
{
  // BFF response shape for Staffing Slot Management
  // Aggregated from service calls above
  "data": {
    // Primary entity data from ps-res-svc
  },
  "metadata": {
    "totalCount": 0,
    "page": 1,
...
|---------|--------|------------|
| `ps-res-svc` unavailable | Feature non-functional | Show error state with retry |
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-005-02.aui.yaml`.
...
- **AC-F-PS-005-02-01:** Given the preconditions for 'Happy path: create staffing slot', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-005-02-02:** Given the preconditions for 'Fill slot with named resource', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-005-02-03:** Given the preconditions for 'Release resource', when the user performs the described action, then the system behaves as specified in Scenario 3.
- **AC-F-PS-005-02-04:** Given the preconditions for 'Invalid FTE', when the user performs the described action, then the system behaves as specified in Scenario 4.

- **AC-F-PS-005-02-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-005-02-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-005-02-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-005-02.uvl)* | | |
