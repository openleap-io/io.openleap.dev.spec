...
> **Companion files:** `F-PS-004-03.uvl` (§9), `F-PS-004-03.aui.yaml` (§6, future), `F-PS-004-03.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-004-03`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-004` — see `F-PS-004.md`
> - **Companion UVL:** `F-PS-004-03.uvl`
> - **Companion AUI:** `F-PS-004-03.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-004-03.attrs.md` *(if needed)*
...
This feature lets a manager review, approve, or reject submitted time bookings and cost entries, so that only validated data flows into budget actuals.
...
- Main navigation → Approvals
- Notification → pending approval

### 0.4 Exit Points

- All approved → queue empty
- Rejected → notification to booking owner
...
#### Scenario 1: Happy path: approve entries

**Given:** The user has appropriate access permissions and the system is operational.
**When:** Manager opens Approval Queue. System shows all SUBMITTED time bookings and cost entries for projects where the user has APPROVER role. Manager reviews entries (checking hours, dates, WP assignment, comments) and clicks Approve. System moves entries to APPROVED status, publishes ps.tim.timebooking.approved and ps.tim.costentry.approved events, and budget actuals are updated.
...
**Given:** The user has appropriate access permissions and the system is operational.
**When:** Manager selects an entry and clicks Reject. System prompts for a rejection reason (required). Manager enters reason and confirms. Entry moves to REJECTED status. The booking owner is notified and can edit and resubmit.
**Then:** The expected outcome is achieved as described above.
...
**Given:** The user has appropriate access permissions and the system is operational.
**When:** Manager selects multiple entries and clicks 'Approve Selected'. System approves all selected entries in one operation.
**Then:** The expected outcome is achieved as described above.
...
**Given:** The user has appropriate access permissions and the system is operational.
**When:** Manager tries to modify an already approved entry. System prevents it (BR-TIM-005) and suggests creating a correcting entry instead.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Time/Cost Approval Workflow
    UI->>SVC: GET /api/ps/tim/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Time/Cost Approval Workflow screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Time/Cost Approval Workflow
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-004-03.aui.yaml`.
...
- **AC-F-PS-004-03-01:** Given the preconditions for 'Happy path: approve entries', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-004-03-02:** Given the preconditions for 'Reject with reason', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-004-03-03:** Given the preconditions for 'Bulk approve', when the user performs the described action, then the system behaves as specified in Scenario 3.
- **AC-F-PS-004-03-04:** Given the preconditions for 'Immutability check', when the user performs the described action, then the system behaves as specified in Scenario 4.

- **AC-F-PS-004-03-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-004-03-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-004-03-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-004-03.uvl)* | | |
