...
# F-PS-003-02 — Cost Type Management

> **Conceptual Stack Layer:** Platform-Feature
> **Space:** Platform
> **Owner:** Domain Engineering Team
> **Companion files:** `F-PS-003-02.uvl` (§9), `F-PS-003-02.aui.yaml` (§6, future), `F-PS-003-02.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-003-02`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-003` — see `F-PS-003.md`
> - **Companion UVL:** `F-PS-003-02.uvl`
> - **Companion AUI:** `F-PS-003-02.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-003-02.attrs.md` *(if needed)*
...
This feature lets an admin define and manage cost type classifications with units and rate schedules, so that costs are categorized consistently across projects.
...
- Settings → Cost Types
- Budget Planning → 'Manage Cost Types' link

### 0.4 Exit Points

- Cost type saved → return to catalog
...
#### Scenario 1: Happy path: create cost type

**Given:** The user has appropriate access permissions and the system is operational.
**When:** Admin opens Cost Type catalog. Admin clicks 'Create'. System shows form: code (e.g., 'TRAVEL'), name ('Travel Expenses'), description, unit ('km'). Admin saves. Admin then adds rate schedule entries: effective from, effective to (optional), rate, currency.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Edit rate schedule

**Given:** The user has appropriate access permissions and the system is operational.
**When:** Admin adds a new rate effective from 2027-01-01 for an existing cost type. System validates no overlap with existing rate periods (BR-BUD-007) and saves.
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: Deactivate cost type

**Given:** The user has appropriate access permissions and the system is operational.
**When:** Admin deactivates a cost type no longer used. System sets isActive=false. The cost type no longer appears in dropdowns for new entries but historical data is preserved.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Cost Type Management
    UI->>SVC: GET /api/ps/bud/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Cost Type Management screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Cost Type Management
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-003-02.aui.yaml`.
...
- **AC-F-PS-003-02-01:** Given the preconditions for 'Happy path: create cost type', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-003-02-02:** Given the preconditions for 'Edit rate schedule', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-003-02-03:** Given the preconditions for 'Deactivate cost type', when the user performs the described action, then the system behaves as specified in Scenario 3.

- **AC-F-PS-003-02-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-003-02-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-003-02-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-003-02.uvl)* | | |
