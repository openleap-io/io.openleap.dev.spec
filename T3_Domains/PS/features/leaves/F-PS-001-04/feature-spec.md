...
> **Companion files:** `F-PS-001-04.uvl` (§9), `F-PS-001-04.aui.yaml` (§6, future), `F-PS-001-04.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-001-04`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-001` — see `F-PS-001.md`
> - **Companion UVL:** `F-PS-001-04.uvl`
> - **Companion AUI:** `F-PS-001-04.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-001-04.attrs.md` *(if needed)*
...
This feature lets a Project Manager visualize and manipulate the project schedule as a Gantt chart with dependency arrows, critical path highlighting, and auto-scheduling, so that the timeline is managed effectively.
...
- Project Detail → 'Gantt' tab

### 0.4 Exit Points

- Click WP bar → Work Package Detail (F-PS-001-05)
- Switch to WBS tab (F-PS-001-03)
...
#### Scenario 1: Happy path: view Gantt

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM opens the Gantt tab. System renders all work packages as horizontal bars on a calendar timeline. Dependencies are shown as arrows between bars. The critical path is highlighted in red. Milestones appear as diamond markers. PM can zoom in/out (day/week/month view) and scroll horizontally.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Drag to reschedule

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM drags a work package bar to a new date range. System updates planned start and planned end. If auto-scheduling is enabled, system recalculates dependent WP dates using forward pass scheduling.
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: Auto-schedule

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM clicks 'Auto-Schedule'. System runs forward pass scheduling from project start date, respecting all dependencies and durations, using the working calendar. System updates all WP dates and highlights the critical path. Any scheduling conflicts (e.g., past-due milestones) are flagged with warning icons.
**Then:** The expected outcome is achieved as described above.


#### Scenario 4: Baseline overlay

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM selects a baseline from a dropdown. System overlays the baseline bars (in gray) behind the current bars, allowing visual comparison of schedule drift.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Gantt Chart & Scheduling
    UI->>SVC: GET /api/ps/prj/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Gantt Chart & Scheduling screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Gantt Chart & Scheduling
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-001-04.aui.yaml`.
...
- **AC-F-PS-001-04-01:** Given the preconditions for 'Happy path: view Gantt', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-001-04-02:** Given the preconditions for 'Drag to reschedule', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-001-04-03:** Given the preconditions for 'Auto-schedule', when the user performs the described action, then the system behaves as specified in Scenario 3.
- **AC-F-PS-001-04-04:** Given the preconditions for 'Baseline overlay', when the user performs the described action, then the system behaves as specified in Scenario 4.

- **AC-F-PS-001-04-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-001-04-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-001-04-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-001-04.uvl)* | | |
