...
> **Companion files:** `F-PS-002-04.uvl` (§9), `F-PS-002-04.aui.yaml` (§6, future), `F-PS-002-04.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-002-04`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-002` — see `F-PS-002.md`
> - **Companion UVL:** `F-PS-002-04.uvl`
> - **Companion AUI:** `F-PS-002-04.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-002-04.attrs.md` *(if needed)*
...
This feature lets a Scrum Master view a burndown chart for the active sprint showing remaining work over time, so that the team can see whether they are on track to complete all committed work.
...
- Sprint Detail → 'Burndown' tab
- Agile Dashboard

### 0.4 Exit Points

- Click data point → sprint detail for that day
...
#### Scenario 1: Happy path: view burndown

**Given:** The user has appropriate access permissions and the system is operational.
**When:** SM opens the Burndown tab for the active sprint. System shows a line chart: X-axis = sprint days, Y-axis = remaining story points (or WP count). An ideal burndown line shows the target trajectory. The actual burndown line shows daily remaining work. If actual is above ideal, the sprint is behind.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: No active sprint

**Given:** The user has appropriate access permissions and the system is operational.
**When:** SM opens Burndown with no active sprint. System shows historical burndown for the most recently closed sprint, or an empty state if no sprints have been completed.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Burndown Chart
    UI->>SVC: GET /api/ps/agl/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Burndown Chart screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Burndown Chart
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-002-04.aui.yaml`.
...
- **AC-F-PS-002-04-01:** Given the preconditions for 'Happy path: view burndown', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-002-04-02:** Given the preconditions for 'No active sprint', when the user performs the described action, then the system behaves as specified in Scenario 2.

- **AC-F-PS-002-04-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-002-04-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-002-04-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-002-04.uvl)* | | |
