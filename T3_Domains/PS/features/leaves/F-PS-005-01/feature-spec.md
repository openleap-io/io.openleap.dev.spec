...
> **Companion files:** `F-PS-005-01.uvl` (§9), `F-PS-005-01.aui.yaml` (§6, future), `F-PS-005-01.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-005-01`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-005` — see `F-PS-005.md`
> - **Companion UVL:** `F-PS-005-01.uvl`
> - **Companion AUI:** `F-PS-005-01.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-005-01.attrs.md` *(if needed)*
...
This feature lets a Resource Manager create and maintain persona (skill/role) profiles that define abstract resource demands, so that projects can plan staffing without requiring specific people.
...
- Settings → Personas
- Staffing Slot → 'Create Persona' shortcut

### 0.4 Exit Points

- Persona saved → persona catalog
...
#### Scenario 1: Happy path: create persona

**Given:** The user has appropriate access permissions and the system is operational.
**When:** RM opens Persona catalog. RM clicks 'Create Persona'. System shows form: name (e.g., 'Senior Java Developer'), description, skills (tag input: 'Java', 'Spring Boot', 'PostgreSQL'), seniority level (dropdown), default hourly rate. RM saves. Persona is available for staffing slots across all projects.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Edit persona

**Given:** The user has appropriate access permissions and the system is operational.
**When:** RM updates a persona's skills or rate. System saves changes. Existing staffing slots referencing this persona are not affected (they snapshot the persona at assignment time).
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: Deactivate persona

**Given:** The user has appropriate access permissions and the system is operational.
**When:** RM deactivates a persona no longer needed. System sets isActive=false. Existing slots are preserved but new slots cannot reference this persona.
**Then:** The expected outcome is achieved as described above.
...
**Given:** The user has appropriate access permissions and the system is operational.
**When:** RM creates a persona with a name that already exists. System rejects (BR-RES-001) with error message.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Persona Management
    UI->>SVC: GET /api/ps/res/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Persona Management screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Persona Management
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-005-01.aui.yaml`.
...
- **AC-F-PS-005-01-01:** Given the preconditions for 'Happy path: create persona', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-005-01-02:** Given the preconditions for 'Edit persona', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-005-01-03:** Given the preconditions for 'Deactivate persona', when the user performs the described action, then the system behaves as specified in Scenario 3.
- **AC-F-PS-005-01-04:** Given the preconditions for 'Duplicate name', when the user performs the described action, then the system behaves as specified in Scenario 4.

- **AC-F-PS-005-01-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-005-01-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-005-01-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-005-01.uvl)* | | |
