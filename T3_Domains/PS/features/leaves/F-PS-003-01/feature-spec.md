...
# F-PS-003-01 — Budget Planning

> **Conceptual Stack Layer:** Platform-Feature
> **Space:** Platform
> **Owner:** Domain Engineering Team
> **Companion files:** `F-PS-003-01.uvl` (§9), `F-PS-003-01.aui.yaml` (§6, future), `F-PS-003-01.attrs.md` (§9, if needed)
> **Referenced by:** Product Spec SS17 (Feature Selection), Suite Feature Catalog SS6
> **References:** Domain/Service Spec `ps_bud-spec.md` (SS5 backend deps), Suite Spec `_ps_suite.md`
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-003-01`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-003` — see `F-PS-003.md`
> - **Companion UVL:** `F-PS-003-01.uvl`
> - **Companion AUI:** `F-PS-003-01.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-003-01.attrs.md` *(if needed)*
...
This feature lets a Controller or PM create and manage project budgets with planned labor and unit cost lines, so that the financial plan for the project is established.
...
- Budget saved → Budget vs. Actuals (F-PS-003-03)
- Add cost type → Cost Type Management (F-PS-003-02)
...
#### Scenario 1: Happy path: create budget

**Given:** The user has appropriate access permissions and the system is operational.
**When:** Controller opens Budget tab for an active project. System shows budget creation form: name, currency. Controller saves. System creates budget in DRAFT status. Controller adds planned labor cost lines (persona/role, hours, hourly rate) and planned unit cost lines (cost type, quantity, unit rate). System auto-calculates totalPlanned as the sum of all lines.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Activate budget

**Given:** The user has appropriate access permissions and the system is operational.
**When:** Controller reviews the budget plan and clicks 'Activate'. System moves budget to ACTIVE status. Budget vs. actuals tracking begins. Actual costs from ps-tim-svc will now be accumulated.
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: Edit planned amounts

**Given:** The user has appropriate access permissions and the system is operational.
**When:** Controller adjusts planned hours for a labor line. System recalculates the line amount and total planned. If the budget is ACTIVE, changes are audit-logged.
**Then:** The expected outcome is achieved as described above.
...
**Given:** The user has appropriate access permissions and the system is operational.
**When:** Controller tries to create a second active budget (BR-BUD-002). System prevents it and suggests closing the existing budget first.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    participant SVC as ps-bud-svc
    participant EVT as Event Bus

    U->>UI: Navigate to Budget Planning
    UI->>SVC: GET /api/ps/bud/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Budget Planning screen
    U->>UI: Perform action
    UI->>SVC: POST/PATCH /api/ps/bud/v1/...
...
Fields are defined by the aggregate attributes in `ps_bud-spec.md`. Key fields for this feature:
...
|---------|------|----------|--------|------------|--------------|
| `ps-bud-svc` | T3 | `/api/ps/bud/v1/...` | GET/POST/PATCH | Read: No, Write: Yes | Retry with backoff; degrade to cached data for reads |
...
```jsonc
{
  // BFF response shape for Budget Planning
  // Aggregated from service calls above
  "data": {
    // Primary entity data from ps-bud-svc
  },
  "metadata": {
    "totalCount": 0,
    "page": 1,
...
|---------|--------|------------|
| `ps-bud-svc` unavailable | Feature non-functional | Show error state with retry |
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-003-01.aui.yaml`.
...
- **AC-F-PS-003-01-01:** Given the preconditions for 'Happy path: create budget', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-003-01-02:** Given the preconditions for 'Activate budget', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-003-01-03:** Given the preconditions for 'Edit planned amounts', when the user performs the described action, then the system behaves as specified in Scenario 3.
- **AC-F-PS-003-01-04:** Given the preconditions for 'Multiple budgets', when the user performs the described action, then the system behaves as specified in Scenario 4.

- **AC-F-PS-003-01-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-003-01-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-003-01-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-003-01.uvl)* | | |
