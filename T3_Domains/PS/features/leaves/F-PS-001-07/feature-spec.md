...
> **Companion files:** `F-PS-001-07.uvl` (§9), `F-PS-001-07.aui.yaml` (§6, future), `F-PS-001-07.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-001-07`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-001` — see `F-PS-001.md`
> - **Companion UVL:** `F-PS-001-07.uvl`
> - **Companion AUI:** `F-PS-001-07.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-001-07.attrs.md` *(if needed)*
...
This feature lets a Project Manager track milestones across the project with planned and actual dates, critical path status, and achievement history, so that key deliverables and decision points are monitored.
...
- Click milestone → Work Package Detail (F-PS-001-05)
...
#### Scenario 1: Happy path: milestone overview

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM opens the Milestones tab. System shows a table of all milestones (WPs with type=MILESTONE) with columns: code, subject, planned date, actual date, status, is on critical path. Milestones are sorted by planned date.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Mark milestone achieved

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM clicks 'Achieved' on a milestone. System sets actualDate to today, updates status to DONE, and publishes ps.prj.milestone.achieved event. The milestone turns green in the Gantt chart.
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: Overdue milestone warning

**Given:** The user has appropriate access permissions and the system is operational.
**When:** System highlights milestones where plannedDate is in the past and actualDate is null. These appear with a warning icon and red text in the milestone table and on the Gantt chart.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Milestone Tracking
    UI->>SVC: GET /api/ps/prj/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Milestone Tracking screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Milestone Tracking
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-001-07.aui.yaml`.
...
- **AC-F-PS-001-07-01:** Given the preconditions for 'Happy path: milestone overview', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-001-07-02:** Given the preconditions for 'Mark milestone achieved', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-001-07-03:** Given the preconditions for 'Overdue milestone warning', when the user performs the described action, then the system behaves as specified in Scenario 3.

- **AC-F-PS-001-07-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-001-07-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-001-07-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-001-07.uvl)* | | |
