...
> **Companion files:** `F-PS-001-09.uvl` (§9), `F-PS-001-09.aui.yaml` (§6, future), `F-PS-001-09.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-001-09`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-001` — see `F-PS-001.md`
> - **Companion UVL:** `F-PS-001-09.uvl`
> - **Companion AUI:** `F-PS-001-09.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-001-09.attrs.md` *(if needed)*
...
This feature lets a PM or admin create, manage, and instantiate reusable project templates with predefined WBS, milestones, and configuration, so that new projects can be set up quickly and consistently.
...
- Template saved → Template list
- Instantiated → new project WBS Editor
...
#### Scenario 1: Happy path: create template

**Given:** The user has appropriate access permissions and the system is operational.
**When:** Admin opens the Templates section. Admin clicks 'Create Template'. System shows a form for template name, description, and an option to create from scratch or save an existing project as a template. Admin saves. System creates the template with a snapshot of the project's WBS, milestones, and configuration.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Instantiate template

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM creates a new project and selects 'From Template'. System shows available templates (tenant-specific and global). PM selects a template. System copies the template WBS, milestones, and default configuration into the new project. PM adjusts dates and attributes, then saves.
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: Manage global templates

**Given:** The user has appropriate access permissions and the system is operational.
**When:** Platform admin creates a global template (isGlobal=true) that is available to all tenants. Tenant admins can view but not modify global templates.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Project Templates
    UI->>SVC: GET /api/ps/prj/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Project Templates screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Project Templates
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-001-09.aui.yaml`.
...
- **AC-F-PS-001-09-01:** Given the preconditions for 'Happy path: create template', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-001-09-02:** Given the preconditions for 'Instantiate template', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-001-09-03:** Given the preconditions for 'Manage global templates', when the user performs the described action, then the system behaves as specified in Scenario 3.

- **AC-F-PS-001-09-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-001-09-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-001-09-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-001-09.uvl)* | | |
