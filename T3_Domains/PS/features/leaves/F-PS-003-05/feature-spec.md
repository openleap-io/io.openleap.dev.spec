...
# F-PS-003-05 — Cost Forecasting (EAC/ETC)

> **Conceptual Stack Layer:** Platform-Feature
> **Space:** Platform
> **Owner:** Domain Engineering Team
> **Companion files:** `F-PS-003-05.uvl` (§9), `F-PS-003-05.aui.yaml` (§6, future), `F-PS-003-05.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-003-05`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-003` — see `F-PS-003.md`
> - **Companion UVL:** `F-PS-003-05.uvl`
> - **Companion AUI:** `F-PS-003-05.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-003-05.attrs.md` *(if needed)*
...
This feature lets a Controller view cost forecasts including Estimate at Completion (EAC) and Estimate to Complete (ETC), so that the final project cost can be predicted and managed.
...
- EVM Dashboard → 'Forecasting' tab

### 0.4 Exit Points

- Adjust budget → Budget Planning (F-PS-003-01)
...
#### Scenario 1: Happy path: view forecasts

**Given:** The user has appropriate access permissions and the system is operational.
**When:** Controller opens Forecasting tab. System shows: EAC (multiple methods: EAC = BAC/CPI, EAC = AC + (BAC-EV)/CPI, EAC = AC + bottom-up ETC), ETC (= EAC - AC), Variance at Completion (VAC = BAC - EAC). System displays these as a summary table and a chart projecting cumulative cost to completion.
...
**Given:** The user has appropriate access permissions and the system is operational.
**When:** Controller toggles between EAC calculation methods. System recalculates and shows the different projections side by side so the controller can choose the most appropriate forecast.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Cost Forecasting (EAC/ETC)
    UI->>SVC: GET /api/ps/bud/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Cost Forecasting (EAC/ETC) screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Cost Forecasting (EAC/ETC)
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-003-05.aui.yaml`.
...
- **AC-F-PS-003-05-01:** Given the preconditions for 'Happy path: view forecasts', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-003-05-02:** Given the preconditions for 'Compare methods', when the user performs the described action, then the system behaves as specified in Scenario 2.

- **AC-F-PS-003-05-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-003-05-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-003-05-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-003-05.uvl)* | | |
