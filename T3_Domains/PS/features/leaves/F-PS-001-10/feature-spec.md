...
> **Companion files:** `F-PS-001-10.uvl` (§9), `F-PS-001-10.aui.yaml` (§6, future), `F-PS-001-10.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-001-10`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-001` — see `F-PS-001.md`
> - **Companion UVL:** `F-PS-001-10.uvl`
> - **Companion AUI:** `F-PS-001-10.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-001-10.attrs.md` *(if needed)*
...
This feature lets a user visualize all relationships of a work package including dependencies, parent-child hierarchy, sprint assignment, budget assignment, and staffing, so that the full context of a work package is visible in one view.
...
- Work Package Detail → 'Relations' tab

### 0.4 Exit Points

- Click related WP → Work Package Detail (F-PS-001-05)
...
#### Scenario 1: Happy path: view relations

**Given:** The user has appropriate access permissions and the system is operational.
**When:** User opens a WP and navigates to the 'Relations' tab. System shows a graph or structured view of: parent WP, child WPs, predecessor dependencies (with type and lag), successor dependencies, sprint assignment (from ps-agl-svc), budget line assignment (from ps-bud-svc), staffing slot (from ps-res-svc).
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Navigate via relations

**Given:** The user has appropriate access permissions and the system is operational.
**When:** User clicks a related WP node in the graph. System navigates to that WP's detail view.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Work Package Relations View
    UI->>SVC: GET /api/ps/prj/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Work Package Relations View screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Work Package Relations View
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-001-10.aui.yaml`.
...
- **AC-F-PS-001-10-01:** Given the preconditions for 'Happy path: view relations', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-001-10-02:** Given the preconditions for 'Navigate via relations', when the user performs the described action, then the system behaves as specified in Scenario 2.

- **AC-F-PS-001-10-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-001-10-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-001-10-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-001-10.uvl)* | | |
