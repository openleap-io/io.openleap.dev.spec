...
> **Companion files:** `F-PS-002-06.uvl` (§9), `F-PS-002-06.aui.yaml` (§6, future), `F-PS-002-06.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-002-06`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-002` — see `F-PS-002.md`
> - **Companion UVL:** `F-PS-002-06.uvl`
> - **Companion AUI:** `F-PS-002-06.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-002-06.attrs.md` *(if needed)*
...
This feature lets a user create custom board views organized by assignee, version, or custom criteria rather than status, so that work can be visualized from different perspectives.
...
#### Scenario 1: Happy path: create custom board

**Given:** The user has appropriate access permissions and the system is operational.
**When:** User clicks 'Create Board'. System prompts for board name and type (Assignee, Version, Custom). For an Assignee board, columns are automatically generated per team member. For Custom, user defines columns with filter criteria. User saves the board.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Assignee board

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM opens an Assignee board. System shows one column per team member assigned to the project, plus an 'Unassigned' column. Cards are distributed by the WP assignee field. Dragging a card between columns reassigns the WP.
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: Version board

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM creates a Version board. Columns represent project versions or releases. WPs are grouped by their target version field. This gives a release planning view.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Custom Boards (Assignee/Version)
    UI->>SVC: GET /api/ps/agl/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Custom Boards (Assignee/Version) screen
    U->>UI: Perform action
...
```
┌─────────────────────────────────────────────────┐
│  Custom Boards (Assignee/Version)                                         │
│  ─────────────────────────────────────────────── │
│                                                   │
...
```jsonc
{
  // BFF response shape for Custom Boards (Assignee/Version)
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-002-06.aui.yaml`.
...
- **AC-F-PS-002-06-01:** Given the preconditions for 'Happy path: create custom board', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-002-06-02:** Given the preconditions for 'Assignee board', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-002-06-03:** Given the preconditions for 'Version board', when the user performs the described action, then the system behaves as specified in Scenario 3.

- **AC-F-PS-002-06-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-002-06-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-002-06-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-002-06.uvl)* | | |
