...
> **Companion files:** `F-PS-006-03.uvl` (§9), `F-PS-006-03.aui.yaml` (§6, future), `F-PS-006-03.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-006-03`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-006` — see `F-PS-006.md`
> - **Companion UVL:** `F-PS-006-03.uvl`
> - **Companion AUI:** `F-PS-006-03.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-006-03.attrs.md` *(if needed)*
...
This feature lets an admin define configurable project lifecycles with phases and gates, and assign them to projects, so that governance structure is standardized.
...
- Settings → Lifecycles
- Project Create → lifecycle selector

### 0.4 Exit Points

- Lifecycle saved → lifecycle list
- Assigned to project → project updated
...
#### Scenario 1: Happy path: create lifecycle

**Given:** The user has appropriate access permissions and the system is operational.
**When:** Admin opens Lifecycle Management. Clicks 'Create Lifecycle'. System shows a visual editor for defining phases and gates in sequence. Admin adds phases (Initiation → Planning → Execution → Closure) with entry/exit gates (Feasibility Gate, Launch Gate, etc.). Admin defines required deliverables and review criteria for each gate. Admin saves.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Assign lifecycle to project

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM opens a project and selects a lifecycle template. System assigns the lifecycle and creates the corresponding phases in the project. Publishes ps.prt.lifecycle.assigned event.
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: Default lifecycle

**Given:** The user has appropriate access permissions and the system is operational.
**When:** Admin sets a lifecycle as default (BR-PRT-004). All new projects automatically use this lifecycle unless overridden.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Lifecycle & Gate Management
    UI->>SVC: GET /api/ps/prt/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Lifecycle & Gate Management screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Lifecycle & Gate Management
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-006-03.aui.yaml`.
...
- **AC-F-PS-006-03-01:** Given the preconditions for 'Happy path: create lifecycle', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-006-03-02:** Given the preconditions for 'Assign lifecycle to project', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-006-03-03:** Given the preconditions for 'Default lifecycle', when the user performs the described action, then the system behaves as specified in Scenario 3.

- **AC-F-PS-006-03-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-006-03-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-006-03-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-006-03.uvl)* | | |
