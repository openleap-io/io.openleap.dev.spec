...
> **Companion files:** `F-PS-006-02.uvl` (§9), `F-PS-006-02.aui.yaml` (§6, future), `F-PS-006-02.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-006-02`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-006` — see `F-PS-006.md`
> - **Companion UVL:** `F-PS-006-02.uvl`
> - **Companion AUI:** `F-PS-006-02.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-006-02.attrs.md` *(if needed)*
...
This feature lets a Program Manager create and manage programs that group related projects, so that cross-project coordination and benefit realization are tracked.
...
- Portfolio Dashboard → click program

### 0.4 Exit Points

- Click project → Project detail
- Click portfolio → Portfolio Dashboard
...
#### Scenario 1: Happy path: create program

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PGM Manager opens Programs. Clicks 'Create Program'. System shows form: name, description, manager (IAM user lookup). PGM saves. System creates program in ACTIVE status.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Add projects to program

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PGM opens a program and clicks 'Add Projects'. System shows eligible projects (same tenant, BR-PRT-003). PGM selects projects and confirms. System updates program membership.
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: Program overview

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PGM opens a program. System shows: member projects with their status, combined timeline (earliest start to latest end), combined budget summary, cross-project dependencies (WP dependencies spanning projects in the program).
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Program Management
    UI->>SVC: GET /api/ps/prt/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Program Management screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Program Management
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-006-02.aui.yaml`.
...
- **AC-F-PS-006-02-01:** Given the preconditions for 'Happy path: create program', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-006-02-02:** Given the preconditions for 'Add projects to program', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-006-02-03:** Given the preconditions for 'Program overview', when the user performs the described action, then the system behaves as specified in Scenario 3.

- **AC-F-PS-006-02-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-006-02-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-006-02-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-006-02.uvl)* | | |
