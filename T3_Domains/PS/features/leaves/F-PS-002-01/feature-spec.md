...
> **Companion files:** `F-PS-002-01.uvl` (§9), `F-PS-002-01.aui.yaml` (§6, future), `F-PS-002-01.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-002-01`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-002` — see `F-PS-002.md`
> - **Companion UVL:** `F-PS-002-01.uvl`
> - **Companion AUI:** `F-PS-002-01.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-002-01.attrs.md` *(if needed)*
...
This feature lets a Product Owner view, order, and estimate the product backlog of work packages, so that the team always knows the highest-priority work to do next.
...
- Project Detail → 'Backlog' tab
- Main navigation → Backlogs

### 0.4 Exit Points

- Click WP → Work Package Detail (F-PS-001-05)
- Select items for sprint → Sprint Planning (F-PS-002-02)
...
#### Scenario 1: Happy path: view and reorder

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PO opens the Product Backlog for a project. System shows an ordered list of all work packages in the product backlog with columns: position, code, subject, type, story points, status. PO can drag-and-drop items to reorder priority. Changes are saved immediately.
...
**Given:** The user has appropriate access permissions and the system is operational.
**When:** PO selects a WP and enters story points. System saves the estimation. The total story points for the backlog are shown at the top.
**Then:** The expected outcome is achieved as described above.
...
**Given:** The user has appropriate access permissions and the system is operational.
**When:** PO filters the backlog by WP type (e.g., only User Stories), status (e.g., only NEW), or searches by subject text.
**Then:** The expected outcome is achieved as described above.


#### Scenario 4: Add items to backlog

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PO clicks 'Add to Backlog'. System shows WPs from the project that are not yet in any backlog. PO selects items and they are added to the bottom of the product backlog.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Product Backlog
    UI->>SVC: GET /api/ps/agl/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Product Backlog screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Product Backlog
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-002-01.aui.yaml`.
...
- **AC-F-PS-002-01-01:** Given the preconditions for 'Happy path: view and reorder', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-002-01-02:** Given the preconditions for 'Estimate story points', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-002-01-03:** Given the preconditions for 'Filter and search', when the user performs the described action, then the system behaves as specified in Scenario 3.
- **AC-F-PS-002-01-04:** Given the preconditions for 'Add items to backlog', when the user performs the described action, then the system behaves as specified in Scenario 4.

- **AC-F-PS-002-01-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-002-01-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-002-01-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-002-01.uvl)* | | |
