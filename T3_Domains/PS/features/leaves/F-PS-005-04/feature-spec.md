...
> **Companion files:** `F-PS-005-04.uvl` (§9), `F-PS-005-04.aui.yaml` (§6, future), `F-PS-005-04.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-005-04`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-005` — see `F-PS-005.md`
> - **Companion UVL:** `F-PS-005-04.uvl`
> - **Companion AUI:** `F-PS-005-04.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-005-04.attrs.md` *(if needed)*
...
This feature lets a Resource Manager view aggregated resource demand across all projects or a portfolio, grouped by persona/skill, so that over-allocation and capacity gaps are identified.
...
- Main navigation → Resource Demand
- Portfolio Dashboard → 'Resource Demand' widget

### 0.4 Exit Points

- Click slot → Staffing Slot detail
...
#### Scenario 1: Happy path: view aggregate demand

**Given:** The user has appropriate access permissions and the system is operational.
**When:** RM opens Demand Dashboard. System aggregates all open and filled staffing slots across projects. Shows a stacked bar chart: X-axis = time periods (months), Y-axis = FTE, stacks = personas. A dashed line shows available capacity (from ops-res-svc). Areas where demand exceeds capacity are highlighted in red.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Filter by portfolio

**Given:** The user has appropriate access permissions and the system is operational.
**When:** RM filters to a specific portfolio. System recalculates aggregation for only projects in that portfolio.
**Then:** The expected outcome is achieved as described above.
...
**Given:** The user has appropriate access permissions and the system is operational.
**When:** RM clicks a bar segment (e.g., 'Senior Java Developer in October'). System shows the individual staffing slots contributing to that demand.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Demand Aggregation Dashboard
    UI->>SVC: GET /api/ps/res/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Demand Aggregation Dashboard screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Demand Aggregation Dashboard
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-005-04.aui.yaml`.
...
- **AC-F-PS-005-04-01:** Given the preconditions for 'Happy path: view aggregate demand', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-005-04-02:** Given the preconditions for 'Filter by portfolio', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-005-04-03:** Given the preconditions for 'Drill down', when the user performs the described action, then the system behaves as specified in Scenario 3.

- **AC-F-PS-005-04-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-005-04-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-005-04-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-005-04.uvl)* | | |
