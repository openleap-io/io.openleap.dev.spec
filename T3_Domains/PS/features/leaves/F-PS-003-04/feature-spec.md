...
# F-PS-003-04 — Earned Value Management

> **Conceptual Stack Layer:** Platform-Feature
> **Space:** Platform
> **Owner:** Domain Engineering Team
> **Companion files:** `F-PS-003-04.uvl` (§9), `F-PS-003-04.aui.yaml` (§6, future), `F-PS-003-04.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-003-04`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-003` — see `F-PS-003.md`
> - **Companion UVL:** `F-PS-003-04.uvl`
> - **Companion AUI:** `F-PS-003-04.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-003-04.attrs.md` *(if needed)*
...
This feature lets a Controller view Earned Value Management metrics (PV, EV, AC, SV, CV, SPI, CPI) and trends for a project, so that objective progress and cost performance are measured.
...
- Project Detail → 'EVM' tab

### 0.4 Exit Points

- Create baseline → Baseline Management (F-PS-001-08)
...
#### Scenario 1: Happy path: view EVM

**Given:** The user has appropriate access permissions and the system is operational.
**When:** Controller opens the EVM tab. System shows current EVM metrics: Planned Value (PV), Earned Value (EV), Actual Cost (AC), Schedule Variance (SV = EV-PV), Cost Variance (CV = EV-AC), Schedule Performance Index (SPI = EV/PV), Cost Performance Index (CPI = EV/AC). Traffic light indicators: green (SPI/CPI >= 1.0), yellow (0.9-1.0), red (<0.9).
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: EVM trend chart

**Given:** The user has appropriate access permissions and the system is operational.
**When:** System shows a time-series chart with PV, EV, and AC lines plotted over project duration. EVM snapshots are taken periodically (configurable: daily, weekly, on-demand).
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: No baseline available

**Given:** The user has appropriate access permissions and the system is operational.
**When:** Controller opens EVM for a project with no baseline. System shows an info message explaining that EVM requires a baseline (BR-BUD-005) and provides a link to create one.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Earned Value Management
    UI->>SVC: GET /api/ps/bud/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Earned Value Management screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Earned Value Management
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-003-04.aui.yaml`.
...
- **AC-F-PS-003-04-01:** Given the preconditions for 'Happy path: view EVM', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-003-04-02:** Given the preconditions for 'EVM trend chart', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-003-04-03:** Given the preconditions for 'No baseline available', when the user performs the described action, then the system behaves as specified in Scenario 3.

- **AC-F-PS-003-04-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-003-04-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-003-04-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-003-04.uvl)* | | |
