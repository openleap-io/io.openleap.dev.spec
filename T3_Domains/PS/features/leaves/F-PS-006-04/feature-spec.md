...
> **Companion files:** `F-PS-006-04.uvl` (§9), `F-PS-006-04.aui.yaml` (§6, future), `F-PS-006-04.attrs.md` (§9, if needed)
> **Referenced by:** Product Spec SS17 (Feature Selection), Suite Feature Catalog SS6
> **References:** Domain/Service Spec `ps_prt-spec.md` (SS5 backend deps), Suite Spec `_ps_suite.md`
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-006-04`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-006` — see `F-PS-006.md`
> - **Companion UVL:** `F-PS-006-04.uvl`
> - **Companion AUI:** `F-PS-006-04.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-006-04.attrs.md` *(if needed)*
...
This feature lets a Portfolio Manager conduct gate reviews for projects, evaluating deliverables and deciding to proceed, hold, or cancel, so that projects pass through governance checkpoints.
...
- Portfolio Dashboard → 'Review' button on project at gate
- Project Detail → 'Gate Review'

### 0.4 Exit Points

- PROCEED → project advances to next phase
- HOLD → project stays at current gate
- CANCEL → project cancellation flow
...
#### Scenario 1: Happy path: conduct review

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PFM opens a project at a gate. System shows: gate name, required deliverables (with completion status), review criteria, project health summary (budget, schedule, resources). PFM enters review comments and selects outcome: PROCEED. System records the review, publishes ps.prt.gate.approved, and project advances to the next phase.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Hold decision

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PFM selects HOLD. System records the review. Project remains in current phase. PFM must conduct a new review later to proceed.
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: Cancel decision

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PFM selects CANCEL. System records the review, publishes ps.prt.gate.rejected with cancel intent. Project is flagged for cancellation in ps-prj-svc.
**Then:** The expected outcome is achieved as described above.


#### Scenario 4: Gate order enforcement

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PFM tries to review a gate out of order (BR-PRT-006). System prevents it and shows which gate must be reviewed first.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    participant SVC as ps-prt-svc
    participant EVT as Event Bus

    U->>UI: Navigate to Gate Review Workflow
    UI->>SVC: GET /api/ps/prt/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Gate Review Workflow screen
    U->>UI: Perform action
    UI->>SVC: POST/PATCH /api/ps/prt/v1/...
...
Fields are defined by the aggregate attributes in `ps_prt-spec.md`. Key fields for this feature:
...
|---------|------|----------|--------|------------|--------------|
| `ps-prt-svc` | T3 | `/api/ps/prt/v1/...` | GET/POST/PATCH | Read: No, Write: Yes | Retry with backoff; degrade to cached data for reads |
...
```jsonc
{
  // BFF response shape for Gate Review Workflow
  // Aggregated from service calls above
  "data": {
    // Primary entity data from ps-prt-svc
  },
  "metadata": {
    "totalCount": 0,
    "page": 1,
...
|---------|--------|------------|
| `ps-prt-svc` unavailable | Feature non-functional | Show error state with retry |
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-006-04.aui.yaml`.
...
- **AC-F-PS-006-04-01:** Given the preconditions for 'Happy path: conduct review', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-006-04-02:** Given the preconditions for 'Hold decision', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-006-04-03:** Given the preconditions for 'Cancel decision', when the user performs the described action, then the system behaves as specified in Scenario 3.
- **AC-F-PS-006-04-04:** Given the preconditions for 'Gate order enforcement', when the user performs the described action, then the system behaves as specified in Scenario 4.

- **AC-F-PS-006-04-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-006-04-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-006-04-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-006-04.uvl)* | | |
