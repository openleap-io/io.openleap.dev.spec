...
> **Companion files:** `F-PS-006-06.uvl` (§9), `F-PS-006-06.aui.yaml` (§6, future), `F-PS-006-06.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-006-06`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-006` — see `F-PS-006.md`
> - **Companion UVL:** `F-PS-006-06.uvl`
> - **Companion AUI:** `F-PS-006-06.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-006-06.attrs.md` *(if needed)*
...
This feature lets a Portfolio Manager view a timeline roadmap of multiple projects showing their phases, milestones, and cross-project dependencies, so that the overall portfolio timeline is visible.
...
- Portfolio Dashboard → 'Roadmap' tab
...
#### Scenario 1: Happy path: view roadmap

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PFM opens Roadmap for a portfolio. System shows a Gantt-like timeline: rows = projects, bars = project phases (colored by lifecycle phase), diamond markers = milestones, arrows = cross-project dependencies. PFM can zoom (quarter/half-year/year) and scroll.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Filter by program

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PFM filters to a specific program. System shows only projects in that program.
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: Identify conflicts

**Given:** The user has appropriate access permissions and the system is operational.
**When:** System highlights cross-project dependency conflicts (e.g., Project B depends on a milestone in Project A that is overdue). Conflict areas are shown in red.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Multi-Project Roadmap
    UI->>SVC: GET /api/ps/prt/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Multi-Project Roadmap screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Multi-Project Roadmap
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-006-06.aui.yaml`.
...
- **AC-F-PS-006-06-01:** Given the preconditions for 'Happy path: view roadmap', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-006-06-02:** Given the preconditions for 'Filter by program', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-006-06-03:** Given the preconditions for 'Identify conflicts', when the user performs the described action, then the system behaves as specified in Scenario 3.

- **AC-F-PS-006-06-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-006-06-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-006-06-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-006-06.uvl)* | | |
