...
> **Companion files:** `F-PS-005-03.uvl` (§9), `F-PS-005-03.aui.yaml` (§6, future), `F-PS-005-03.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-005-03`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-005` — see `F-PS-005.md`
> - **Companion UVL:** `F-PS-005-03.uvl`
> - **Companion AUI:** `F-PS-005-03.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-005-03.attrs.md` *(if needed)*
...
This feature lets a PM view the project team as a timeline showing who is assigned, for what period, and at what FTE, so that the team composition is visible at a glance.
...
- Click resource → resource profile (links to ops-res-svc)
...
#### Scenario 1: Happy path: view team

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM opens Team Planner for a project. System shows a horizontal timeline: rows = staffing slots (persona name + assigned resource name if filled), bars = slot duration colored by status (blue = OPEN, green = FILLED, gray = RELEASED). PM can see gaps (unfilled demand) and overlaps.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Hover for details

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM hovers over a bar. System shows a tooltip: persona, skills, FTE percentage, assigned resource, start/end dates.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Team Planner View
    UI->>SVC: GET /api/ps/res/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Team Planner View screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Team Planner View
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-005-03.aui.yaml`.
...
- **AC-F-PS-005-03-01:** Given the preconditions for 'Happy path: view team', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-005-03-02:** Given the preconditions for 'Hover for details', when the user performs the described action, then the system behaves as specified in Scenario 2.

- **AC-F-PS-005-03-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-005-03-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-005-03-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-005-03.uvl)* | | |
