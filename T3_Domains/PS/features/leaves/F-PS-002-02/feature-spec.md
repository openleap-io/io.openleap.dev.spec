...
> **Companion files:** `F-PS-002-02.uvl` (§9), `F-PS-002-02.aui.yaml` (§6, future), `F-PS-002-02.attrs.md` (§9, if needed)
> **Referenced by:** Product Spec SS17 (Feature Selection), Suite Feature Catalog SS6
> **References:** Domain/Service Spec `ps_agl-spec.md` (SS5 backend deps), Suite Spec `_ps_suite.md`
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-002-02`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-002` — see `F-PS-002.md`
> - **Companion UVL:** `F-PS-002-02.uvl`
> - **Companion AUI:** `F-PS-002-02.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-002-02.attrs.md` *(if needed)*
...
This feature lets a Scrum Master create, plan, start, and close sprints, moving work packages from the product backlog to the sprint backlog, so that time-boxed iterations are managed end to end.
...
- Project Detail → 'Sprints' tab
- Agile navigation → Sprint Planning

### 0.4 Exit Points

- Sprint closed → Sprint Review summary
- Click WP → Work Package Detail (F-PS-001-05)
...
#### Scenario 1: Happy path: create and plan sprint

**Given:** The user has appropriate access permissions and the system is operational.
**When:** SM clicks 'Create Sprint'. System prompts for sprint name, start date, end date, and goal. SM saves. System creates sprint in PLANNED status. SM drags items from the product backlog to the sprint backlog. System shows committed story points vs. team velocity for capacity guidance.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Start sprint

**Given:** The user has appropriate access permissions and the system is operational.
**When:** SM clicks 'Start Sprint'. System validates only one active sprint exists (BR-AGL-001), moves status to ACTIVE, records committed story points, and publishes ps.agl.sprint.started event.
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: Close sprint

**Given:** The user has appropriate access permissions and the system is operational.
**When:** SM clicks 'Close Sprint'. System calculates completed story points, updates rolling velocity, returns uncommitted WPs to the product backlog (BR-AGL-004), and publishes ps.agl.sprint.completed event. System shows sprint review summary: committed vs. completed, velocity trend.
**Then:** The expected outcome is achieved as described above.


#### Scenario 4: Sprint already active

**Given:** The user has appropriate access permissions and the system is operational.
**When:** SM tries to start a second sprint. System prevents it with error BR-AGL-001 and shows which sprint is currently active.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    participant SVC as ps-agl-svc
    participant EVT as Event Bus

    U->>UI: Navigate to Sprint Planning & Management
    UI->>SVC: GET /api/ps/agl/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Sprint Planning & Management screen
    U->>UI: Perform action
    UI->>SVC: POST/PATCH /api/ps/agl/v1/...
...
Fields are defined by the aggregate attributes in `ps_agl-spec.md`. Key fields for this feature:
...
|---------|------|----------|--------|------------|--------------|
| `ps-agl-svc` | T3 | `/api/ps/agl/v1/...` | GET/POST/PATCH | Read: No, Write: Yes | Retry with backoff; degrade to cached data for reads |
...
```jsonc
{
  // BFF response shape for Sprint Planning & Management
  // Aggregated from service calls above
  "data": {
    // Primary entity data from ps-agl-svc
  },
  "metadata": {
    "totalCount": 0,
    "page": 1,
...
|---------|--------|------------|
| `ps-agl-svc` unavailable | Feature non-functional | Show error state with retry |
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-002-02.aui.yaml`.
...
- **AC-F-PS-002-02-01:** Given the preconditions for 'Happy path: create and plan sprint', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-002-02-02:** Given the preconditions for 'Start sprint', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-002-02-03:** Given the preconditions for 'Close sprint', when the user performs the described action, then the system behaves as specified in Scenario 3.
- **AC-F-PS-002-02-04:** Given the preconditions for 'Sprint already active', when the user performs the described action, then the system behaves as specified in Scenario 4.

- **AC-F-PS-002-02-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-002-02-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-002-02-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-002-02.uvl)* | | |
