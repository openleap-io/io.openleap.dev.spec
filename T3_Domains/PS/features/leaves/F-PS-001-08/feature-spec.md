...
> **Companion files:** `F-PS-001-08.uvl` (§9), `F-PS-001-08.aui.yaml` (§6, future), `F-PS-001-08.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-001-08`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-001` — see `F-PS-001.md`
> - **Companion UVL:** `F-PS-001-08.uvl`
> - **Companion AUI:** `F-PS-001-08.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-001-08.attrs.md` *(if needed)*
...
This feature lets a Project Manager create, view, and compare baselines (frozen schedule snapshots) against the current plan, so that schedule drift and scope changes are tracked over time.
...
- Project Detail → 'Baselines' tab
- Gantt Chart → 'Overlay Baseline' dropdown

### 0.4 Exit Points

- Close comparison → return to project
...
#### Scenario 1: Happy path: create baseline

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM clicks 'Create Baseline'. System prompts for a name and optional description. PM enters 'Initial Plan' and confirms. System captures a frozen snapshot of all WP dates, effort, and status at this moment. The baseline appears in the baseline list.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Compare baselines

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM opens the baseline comparison view. PM selects two baselines (or one baseline vs. current plan). System shows a side-by-side or overlay comparison of: WP dates, duration changes, added/removed WPs, effort changes. Differences are highlighted in red (delay) or green (improvement).
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: Baseline overlay on Gantt

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM selects a baseline in the Gantt view. System overlays baseline bars (gray, semi-transparent) behind current bars, visually showing which WPs have shifted.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Baseline Management
    UI->>SVC: GET /api/ps/prj/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Baseline Management screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Baseline Management
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-001-08.aui.yaml`.
...
- **AC-F-PS-001-08-01:** Given the preconditions for 'Happy path: create baseline', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-001-08-02:** Given the preconditions for 'Compare baselines', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-001-08-03:** Given the preconditions for 'Baseline overlay on Gantt', when the user performs the described action, then the system behaves as specified in Scenario 3.

- **AC-F-PS-001-08-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-001-08-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-001-08-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-001-08.uvl)* | | |
