...
> **Companion files:** `F-PS-001-03.uvl` (§9), `F-PS-001-03.aui.yaml` (§6, future), `F-PS-001-03.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-001-03`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-001` — see `F-PS-001.md`
> - **Companion UVL:** `F-PS-001-03.uvl`
> - **Companion AUI:** `F-PS-001-03.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-001-03.attrs.md` *(if needed)*
...
This feature lets a Project Manager build and maintain the Work Breakdown Structure by creating, editing, moving, and deleting work packages in a hierarchical tree view, so that the project scope is fully decomposed.
...
- Project Detail → 'WBS' tab
- Project Create → after save

### 0.4 Exit Points

- Click WP → Work Package Detail (F-PS-001-05)
- Switch to Gantt tab (F-PS-001-04)
...
#### Scenario 1: Happy path: build WBS

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM opens WBS Editor for a project. System shows a hierarchical tree of existing work packages (or empty state for new projects). PM can: add a child WP to any node, add a sibling WP, indent/outdent to change hierarchy, drag-and-drop to reorder, expand/collapse branches. Each WP shows: code, subject, type, status, assignee, dates.
...
**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM double-clicks a WP subject in the tree. System enables inline editing of the subject. PM types new text and presses Enter to save, or Escape to cancel.
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: Add milestone

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM selects a node and clicks 'Add Milestone'. System creates a new WP with type=MILESTONE, duration=0, and opens the detail panel for date entry.
**Then:** The expected outcome is achieved as described above.


#### Scenario 4: Move work package

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM drags a WP from one parent to another. System validates (BR-PRJ-004: same project) and updates the hierarchy. Child WPs move with the parent.
**Then:** The expected outcome is achieved as described above.
...
**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM selects a WP with children and clicks Delete. System shows a confirmation dialog warning that child WPs will also be deleted. PM confirms, and the entire subtree is removed.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to WBS Editor
    UI->>SVC: GET /api/ps/prj/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render WBS Editor screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for WBS Editor
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-001-03.aui.yaml`.
...
- **AC-F-PS-001-03-01:** Given the preconditions for 'Happy path: build WBS', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-001-03-02:** Given the preconditions for 'Inline editing', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-001-03-03:** Given the preconditions for 'Add milestone', when the user performs the described action, then the system behaves as specified in Scenario 3.
- **AC-F-PS-001-03-04:** Given the preconditions for 'Move work package', when the user performs the described action, then the system behaves as specified in Scenario 4.
- **AC-F-PS-001-03-05:** Given the preconditions for 'Delete with confirmation', when the user performs the described action, then the system behaves as specified in Scenario 5.

- **AC-F-PS-001-03-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-001-03-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-001-03-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-001-03.uvl)* | | |
