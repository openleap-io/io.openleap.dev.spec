...
> **Companion files:** `F-PS-001-02.uvl` (§9), `F-PS-001-02.aui.yaml` (§6, future), `F-PS-001-02.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-001-02`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-001` — see `F-PS-001.md`
> - **Companion UVL:** `F-PS-001-02.uvl`
> - **Companion AUI:** `F-PS-001-02.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-001-02.attrs.md` *(if needed)*
...
This feature lets a Project Manager create a new project from scratch or from a template, and edit its core attributes, so that the project is properly defined before planning begins.
...
- Project List → 'Create from Template' button
- Project Detail → 'Edit' button

### 0.4 Exit Points

- Save → WBS Editor (F-PS-001-03)
- Cancel → Project List (F-PS-001-01)
...
#### Scenario 1: Happy path: create from scratch

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM clicks 'Create Project'. System presents a form with fields: code (auto-generated, editable), name (required), description, project type (dropdown), priority (dropdown), customer (BP lookup), planned start, planned end, working calendar (dropdown from cal-svc), lifecycle (dropdown from ps-prt-svc). PM fills required fields and clicks Save. System creates project in DRAFT status and navigates to WBS Editor.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Create from template

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM clicks 'Create from Template'. System shows available templates. PM selects a template. System pre-fills the form with template defaults and copies the template WBS. PM adjusts dates and attributes, then saves.
**Then:** The expected outcome is achieved as described above.
...
**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM opens an existing project and clicks Edit. System loads current values into the form. PM modifies attributes (name, dates, priority, etc.) and saves. System validates changes against business rules (BR-PRJ-002, BR-PRJ-003) and updates the project.
**Then:** The expected outcome is achieved as described above.
...
**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM submits a form with planned end before planned start. System highlights the error (BR-PRJ-003) and shows the error message without submitting.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Project Create/Edit
    UI->>SVC: GET /api/ps/prj/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Project Create/Edit screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Project Create/Edit
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-001-02.aui.yaml`.
...
- **AC-F-PS-001-02-01:** Given the preconditions for 'Happy path: create from scratch', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-001-02-02:** Given the preconditions for 'Create from template', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-001-02-03:** Given the preconditions for 'Edit existing project', when the user performs the described action, then the system behaves as specified in Scenario 3.
- **AC-F-PS-001-02-04:** Given the preconditions for 'Validation error', when the user performs the described action, then the system behaves as specified in Scenario 4.

- **AC-F-PS-001-02-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-001-02-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-001-02-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-001-02.uvl)* | | |
