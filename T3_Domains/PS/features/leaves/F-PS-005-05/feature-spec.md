...
> **Companion files:** `F-PS-005-05.uvl` (§9), `F-PS-005-05.aui.yaml` (§6, future), `F-PS-005-05.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-005-05`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-005` — see `F-PS-005.md`
> - **Companion UVL:** `F-PS-005-05.uvl`
> - **Companion AUI:** `F-PS-005-05.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-005-05.attrs.md` *(if needed)*
...
This feature lets a Resource Manager model what-if staffing scenarios by adding hypothetical demand or adjusting existing slots, so that resource impacts of decisions can be evaluated before committing.
...
- Demand Dashboard → 'What-If' button

### 0.4 Exit Points

- Scenario committed → demand updated
- Scenario discarded → no change
...
#### Scenario 1: Happy path: create scenario

**Given:** The user has appropriate access permissions and the system is operational.
**When:** RM clicks 'Create Scenario'. System creates a sandbox copy of current demand. RM adds hypothetical slots (e.g., 'What if we add 2 more developers to Project Alpha for Q4?'). System shows the impact on aggregate demand vs. capacity.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Compare scenarios

**Given:** The user has appropriate access permissions and the system is operational.
**When:** RM creates multiple scenarios. System shows a side-by-side comparison of aggregate demand charts for each scenario. RM selects the best option.
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: Apply scenario

**Given:** The user has appropriate access permissions and the system is operational.
**When:** RM decides to commit a scenario. System creates real staffing slots based on the scenario. Scenario is archived.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Scenario Planning
    UI->>SVC: GET /api/ps/res/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Scenario Planning screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Scenario Planning
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-005-05.aui.yaml`.
...
- **AC-F-PS-005-05-01:** Given the preconditions for 'Happy path: create scenario', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-005-05-02:** Given the preconditions for 'Compare scenarios', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-005-05-03:** Given the preconditions for 'Apply scenario', when the user performs the described action, then the system behaves as specified in Scenario 3.

- **AC-F-PS-005-05-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-005-05-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-005-05-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-005-05.uvl)* | | |
