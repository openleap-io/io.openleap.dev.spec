...
> **Companion files:** `F-PS-006-05.uvl` (§9), `F-PS-006-05.aui.yaml` (§6, future), `F-PS-006-05.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-006-05`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-006` — see `F-PS-006.md`
> - **Companion UVL:** `F-PS-006-05.uvl`
> - **Companion AUI:** `F-PS-006-05.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-006-05.attrs.md` *(if needed)*
...
This feature lets a Portfolio Manager score and rank projects using configurable scoring models, so that investment decisions are based on objective criteria.
...
- Portfolio Dashboard → 'Prioritize' button

### 0.4 Exit Points

- Rankings saved → Portfolio Dashboard updated
...
#### Scenario 1: Happy path: score projects

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PFM opens Prioritization for a portfolio. System shows a scoring matrix: rows = projects, columns = scoring criteria (from the selected scoring model). PFM enters scores for each project × criterion. System calculates weighted total scores and ranks projects automatically.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Configure scoring model

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PFM opens Scoring Model management. Creates a model (e.g., 'RICE'): Reach (weight 0.3), Impact (weight 0.3), Confidence (weight 0.2), Effort (weight 0.2). System validates weights sum to 1.0 (BR-PRT-007).
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: Compare rankings

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PFM applies two different scoring models to the same portfolio. System shows side-by-side rankings so PFM can see how methodology choice affects prioritization.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Project Scoring & Prioritization
    UI->>SVC: GET /api/ps/prt/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Project Scoring & Prioritization screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Project Scoring & Prioritization
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-006-05.aui.yaml`.
...
- **AC-F-PS-006-05-01:** Given the preconditions for 'Happy path: score projects', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-006-05-02:** Given the preconditions for 'Configure scoring model', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-006-05-03:** Given the preconditions for 'Compare rankings', when the user performs the described action, then the system behaves as specified in Scenario 3.

- **AC-F-PS-006-05-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-006-05-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-006-05-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-006-05.uvl)* | | |
