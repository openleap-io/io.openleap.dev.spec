...
# F-PS-003-03 — Budget vs. Actuals Dashboard

> **Conceptual Stack Layer:** Platform-Feature
> **Space:** Platform
> **Owner:** Domain Engineering Team
> **Companion files:** `F-PS-003-03.uvl` (§9), `F-PS-003-03.aui.yaml` (§6, future), `F-PS-003-03.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-003-03`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-003` — see `F-PS-003.md`
> - **Companion UVL:** `F-PS-003-03.uvl`
> - **Companion AUI:** `F-PS-003-03.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-003-03.attrs.md` *(if needed)*
...
This feature lets a Controller or PM compare planned budget amounts against actual costs accumulated from time bookings and cost entries, so that spending is monitored and controlled.
...
- Project Detail → 'Budget' tab → 'Actuals' sub-tab

### 0.4 Exit Points

- Click cost entry → Time/Cost detail
...
#### Scenario 1: Happy path: view dashboard

**Given:** The user has appropriate access permissions and the system is operational.
**When:** Controller opens Budget vs. Actuals for a project. System shows: overall budget health (total planned vs. total actual with percentage), bar chart comparing planned vs. actual per cost line, trend chart showing cumulative actual cost over time with planned burn rate overlay.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Drill down by cost type

**Given:** The user has appropriate access permissions and the system is operational.
**When:** Controller clicks a cost type bar. System shows all actual cost entries for that cost type with details: date, user, WP, quantity, amount.
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: Overspend alert

**Given:** The user has appropriate access permissions and the system is operational.
**When:** Total actual reaches 90% of total planned (overspendThreshold). System highlights the budget in red and shows an alert. If the threshold was just crossed, ps.bud.budget.overspent event was already published.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Budget vs. Actuals Dashboard
    UI->>SVC: GET /api/ps/bud/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Budget vs. Actuals Dashboard screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Budget vs. Actuals Dashboard
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-003-03.aui.yaml`.
...
- **AC-F-PS-003-03-01:** Given the preconditions for 'Happy path: view dashboard', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-003-03-02:** Given the preconditions for 'Drill down by cost type', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-003-03-03:** Given the preconditions for 'Overspend alert', when the user performs the described action, then the system behaves as specified in Scenario 3.

- **AC-F-PS-003-03-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-003-03-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-003-03-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-003-03.uvl)* | | |
