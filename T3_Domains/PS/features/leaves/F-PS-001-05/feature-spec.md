...
> **Companion files:** `F-PS-001-05.uvl` (§9), `F-PS-001-05.aui.yaml` (§6, future), `F-PS-001-05.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-001-05`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-001` — see `F-PS-001.md`
> - **Companion UVL:** `F-PS-001-05.uvl`
> - **Companion AUI:** `F-PS-001-05.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-001-05.attrs.md` *(if needed)*
...
This feature lets a team member or Project Manager view and edit all attributes of a single work package including description, dates, effort, assignment, status, and related information, so that work package details are fully managed.
...
- WBS Editor → click WP
- Gantt Chart → click WP bar
- Board → click WP card
- Search results → click WP

### 0.4 Exit Points

- Close panel → return to previous screen
- Click parent WP → navigate to parent detail
...
#### Scenario 1: Happy path: view details

**Given:** The user has appropriate access permissions and the system is operational.
**When:** User clicks a WP in the WBS tree or Gantt chart. System opens a detail panel (slide-over or full page) showing all WP attributes: code, subject, description, type, status, priority, assignee, planned/actual dates, duration, planned effort, percent complete, story points, parent, dependencies, time bookings summary, cost entries summary, attached files.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Edit attributes

**Given:** The user has appropriate access permissions and the system is operational.
**When:** User with write permission edits WP fields (subject, description, dates, effort, priority, assignee, status). System validates against business rules (BR-PRJ-006 for milestones, BR-PRJ-009 for percent complete) and saves changes.
**Then:** The expected outcome is achieved as described above.
...
**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM changes WP status from NEW to IN_PROGRESS. System validates the transition is allowed and publishes ps.prj.workpackage.updated event. If WP is completed (status=DONE, percentComplete=100), system publishes ps.prj.workpackage.completed event.
**Then:** The expected outcome is achieved as described above.


#### Scenario 4: View activity log

**Given:** The user has appropriate access permissions and the system is operational.
**When:** User scrolls to the activity log section. System shows a chronological list of all changes to this WP (who changed what, when) from the audit log.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Work Package Detail View
    UI->>SVC: GET /api/ps/prj/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Work Package Detail View screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Work Package Detail View
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-001-05.aui.yaml`.
...
- **AC-F-PS-001-05-01:** Given the preconditions for 'Happy path: view details', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-001-05-02:** Given the preconditions for 'Edit attributes', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-001-05-03:** Given the preconditions for 'Status transition', when the user performs the described action, then the system behaves as specified in Scenario 3.
- **AC-F-PS-001-05-04:** Given the preconditions for 'View activity log', when the user performs the described action, then the system behaves as specified in Scenario 4.

- **AC-F-PS-001-05-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-001-05-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-001-05-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-001-05.uvl)* | | |
