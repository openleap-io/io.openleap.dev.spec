...
> **Companion files:** `F-PS-004-05.uvl` (§9), `F-PS-004-05.aui.yaml` (§6, future), `F-PS-004-05.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-004-05`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-004` — see `F-PS-004.md`
> - **Companion UVL:** `F-PS-004-05.uvl`
> - **Companion AUI:** `F-PS-004-05.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-004-05.attrs.md` *(if needed)*
...
This feature lets a team member view a personal dashboard of their own time bookings across all projects with weekly and monthly summaries, so that individuals can track their own time allocation.
...
- Main navigation → My Time
- Home dashboard → 'My Time' widget

### 0.4 Exit Points

- Click entry → Time Booking detail
...
#### Scenario 1: Happy path: view my time

**Given:** The user has appropriate access permissions and the system is operational.
**When:** User opens 'My Time'. System shows: this week's entries as a weekly grid (project/WP rows × day columns), total hours per day, total hours this week, summary of entries pending approval (SUBMITTED) and rejected entries (REJECTED). A monthly calendar view shows total hours per day with color coding (green = 8h, yellow = <8h, red = 0h).
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Quick entry

**Given:** The user has appropriate access permissions and the system is operational.
**When:** User clicks a cell in the weekly grid. System opens a quick-entry popup for that WP + day. User enters hours and saves without leaving the dashboard.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to My Time (Personal Dashboard)
    UI->>SVC: GET /api/ps/tim/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render My Time (Personal Dashboard) screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for My Time (Personal Dashboard)
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-004-05.aui.yaml`.
...
- **AC-F-PS-004-05-01:** Given the preconditions for 'Happy path: view my time', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-004-05-02:** Given the preconditions for 'Quick entry', when the user performs the described action, then the system behaves as specified in Scenario 2.

- **AC-F-PS-004-05-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-004-05-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-004-05-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-004-05.uvl)* | | |
