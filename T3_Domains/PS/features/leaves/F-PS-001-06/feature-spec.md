...
> **Companion files:** `F-PS-001-06.uvl` (§9), `F-PS-001-06.aui.yaml` (§6, future), `F-PS-001-06.attrs.md` (§9, if needed)
...
> - **Status:** DRAFT
> - **Feature ID:** `F-PS-001-06`
> - **Suite:** `ps` — suite-owned, not product-owned
> - **Node type:** LEAF
> - **Parent:** `F-PS-001` — see `F-PS-001.md`
> - **Companion UVL:** `F-PS-001-06.uvl`
> - **Companion AUI:** `F-PS-001-06.aui.yaml` *(to be created)*
> - **Attribute doc:** `F-PS-001-06.attrs.md` *(if needed)*
...
This feature lets a Project Manager create, view, and delete scheduling dependencies between work packages, so that the project schedule respects logical ordering constraints.
...
- Gantt Chart → draw arrow
- Work Package Detail → Dependencies section

### 0.4 Exit Points

- Dependency created → Gantt updates
- Cancel → no change
...
#### Scenario 1: Happy path: create dependency

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM opens the Gantt chart or dependency panel. PM selects a predecessor WP and draws an arrow to a successor WP (drag-and-drop on Gantt) or uses a form to select predecessor/successor and type (FS/SS/FF/SF). System validates no circular dependency would be created (BR-PRJ-005) and saves the dependency. Gantt updates to show the arrow.
**Then:** The expected outcome is achieved as described above.


#### Scenario 2: Set lag/lead time

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM creates a dependency with a lag of +3 days (successor starts 3 working days after predecessor finishes). System stores the lag and adjusts scheduling calculations accordingly.
**Then:** The expected outcome is achieved as described above.


#### Scenario 3: Delete dependency

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM right-clicks a dependency arrow on the Gantt chart and selects 'Remove'. System deletes the dependency and recalculates affected WP dates if auto-scheduling is enabled.
**Then:** The expected outcome is achieved as described above.


#### Scenario 4: Circular dependency rejected

**Given:** The user has appropriate access permissions and the system is operational.
**When:** PM tries to create a dependency that would create a cycle (A→B→C→A). System detects the cycle (BR-PRJ-005), rejects the creation, and shows an error explaining which WPs are in the cycle.
**Then:** The expected outcome is achieved as described above.
...
sequenceDiagram
    ...
    U->>UI: Navigate to Dependency Management
    UI->>SVC: GET /api/ps/prj/v1/...
    SVC-->>UI: 200 OK (data)
    UI-->>U: Render Dependency Management screen
    U->>UI: Perform action
...
```jsonc
{
  // BFF response shape for Dependency Management
...
The AUI screen contract for this feature will be defined in the companion file `F-PS-001-06.aui.yaml`.
...
- **AC-F-PS-001-06-01:** Given the preconditions for 'Happy path: create dependency', when the user performs the described action, then the system behaves as specified in Scenario 1.
- **AC-F-PS-001-06-02:** Given the preconditions for 'Set lag/lead time', when the user performs the described action, then the system behaves as specified in Scenario 2.
- **AC-F-PS-001-06-03:** Given the preconditions for 'Delete dependency', when the user performs the described action, then the system behaves as specified in Scenario 3.
- **AC-F-PS-001-06-04:** Given the preconditions for 'Circular dependency rejected', when the user performs the described action, then the system behaves as specified in Scenario 4.

- **AC-F-PS-001-06-PERM:** Given a user without the required role, when they attempt to access this feature, then unauthorized actions are hidden and the system shows a read-only view (or access denied if no read permission).
- **AC-F-PS-001-06-LOAD:** Given a normal network connection, when the user navigates to this feature, then the content loads within 2 seconds (p95).
- **AC-F-PS-001-06-ERROR:** Given a backend failure, when the user is viewing this feature, then an error state is shown with a retry option and no data is lost from the user's current session.
...
|-----------------|-----------|--------|
| *(Documented in companion F-PS-001-06.uvl)* | | |
