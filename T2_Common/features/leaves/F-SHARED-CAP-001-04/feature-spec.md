            # F-SHARED-CAP-001-04 — Availability Query

            > **Conceptual Stack Layer:** Platform-Feature
            > **Space:** Platform
            > **Owner:** Domain Engineering Team
            > **Companion files:** `F-SHARED-CAP-001-04.uvl`, `F-SHARED-CAP-001-04.aui.yaml`
            ...
            > - **Status:** DRAFT
            > - **Feature ID:** `F-SHARED-CAP-001-04`
            > - **Suite:** `t2`
            > - **Node type:** LEAF
            > - **Parent:** `F-SHARED-CAP-001` — see `F-SHARED-CAP-001.md`
            > - **Companion UVL:** `F-SHARED-CAP-001-04.uvl`
            > - **Companion AUI:** `F-SHARED-CAP-001-04.aui.yaml`
            ...
            This feature lets authorized users query the effective availability of a calendar for a date range so that planners and booking systems get real-time capacity data .
            ...
            **Exit points:**
            ...
            ├── F-SHARED-CAP-001-01  Resource Calendars  [LEAF]
├── F-SHARED-CAP-001-02  Working Patterns  [LEAF]
├── F-SHARED-CAP-001-03  Holiday Management  [LEAF]
├── F-SHARED-CAP-001-04  Availability Query  [LEAF] <-- you are here
```
            ...
            | `F-SHARED-CAP-001-04.uvl` | Companion UVL — attribute schema, cross-suite requires |
            | `F-SHARED-CAP-001-04.aui.yaml` | Companion AUI — screen contract |
            ...
            Query the effective availability of a calendar for a date range so that planners and booking systems get real-time capacity data.

            ### 1.2 User Scenarios


**Scenario 1: Check Machine Availability**
> A Production Planner wants to query a machine's availability for next week because they need to schedule a production order.
> They navigate to the Availability Query screen, fill in the required data, and submit.
> They expect: The system returns available time blocks after subtracting events and holidays.


**Scenario 2: Check Delivery Slots**
> A Sales Representative wants to query available delivery windows for a customer because the customer wants a delivery next Tuesday.
> They navigate to the Availability Query screen, fill in the required data, and submit.
> They expect: The system returns bookable slots for the requested date range.
            ...
            sequenceDiagram
                ...
                participant UI as Availability Query UI
                participant BFF as BFF
                participant SVC as shared-cap-svc

                User->>UI: Navigate to Availability Query
            ...
            **Screen: Availability Query — Default State**

            ```
            ┌─────────────────────────────────────────────────┐
            │  [Calendar Management] > Availability Query              │  ← Breadcrumb
            ├─────────────────────────────────────────────────┤
            │  ┌─ Core Data ─────────────────────────────────┐│
            ...
            Fields are defined authoritatively in `domain-specs/shared_cap-spec.md` §3 (Domain Model) and §6 (REST API). This feature renders the fields relevant to availability query.
            ...
            See companion file: `contracts/aui/F-SHARED-CAP-001-04.aui.yaml`
...
| BOOKING_USER | Y | Y | Y | N |
            ...
            **AC-1: Check Machine Availability**
            - **Given** a Production Planner is on the Availability Query screen
            - **When** they query a machine's availability for next week
            - **Then** The system returns available time blocks after subtracting events and holidays.

            **AC-2: Check Delivery Slots**
            - **Given** a Sales Representative is on the Availability Query screen
            - **When** they query available delivery windows for a customer
            - **Then** The system returns bookable slots for the requested date range.
            ...
            |---|---|---|
            | `F-SHARED-CAP-001-04` | T1 Reference Data | Validation of codes |
            ...
            |---|---|---|---|
            | `ext.f_t2_002_04.customFields` | zone | Additional fields from T3 domains | Zone hidden |
