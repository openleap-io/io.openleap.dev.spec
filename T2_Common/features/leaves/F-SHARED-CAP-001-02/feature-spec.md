            # F-SHARED-CAP-001-02 — Working Patterns

            > **Conceptual Stack Layer:** Platform-Feature
            > **Space:** Platform
            > **Owner:** Domain Engineering Team
            > **Companion files:** `F-SHARED-CAP-001-02.uvl`, `F-SHARED-CAP-001-02.aui.yaml`
            ...
            > - **Status:** DRAFT
            > - **Feature ID:** `F-SHARED-CAP-001-02`
            > - **Suite:** `t2`
            > - **Node type:** LEAF
            > - **Parent:** `F-SHARED-CAP-001` — see `F-SHARED-CAP-001.md`
            > - **Companion UVL:** `F-SHARED-CAP-001-02.uvl`
            > - **Companion AUI:** `F-SHARED-CAP-001-02.aui.yaml`
            ...
            This feature lets authorized users define and apply reusable working patterns (shift templates) to calendars so that recurring working hours are modeled via rrule .
...
├── F-SHARED-CAP-001-02  Working Patterns  [LEAF] <-- you are here
...
```
            ...
            | `F-SHARED-CAP-001-02.uvl` | Companion UVL — attribute schema, cross-suite requires |
            | `F-SHARED-CAP-001-02.aui.yaml` | Companion AUI — screen contract |
            ...
            Define and apply reusable working patterns (shift templates) to calendars so that recurring working hours are modeled via RRULE.

            ### 1.2 User Scenarios


**Scenario 1: Apply Day Shift**
> A Production Planner wants to apply the standard day shift pattern to a machine calendar because the machine runs Mon-Fri 06:00-14:00.
> They navigate to the Working Patterns screen, fill in the required data, and submit.
> They expect: Events are generated from the RRULE for the effective date range.


**Scenario 2: Change to 3-Shift**
> A Plant Manager wants to switch a resource from single to three-shift pattern because production demand increased.
> They navigate to the Working Patterns screen, fill in the required data, and submit.
> They expect: The old pattern is superseded and capacity buckets are rebuilt.
            ...
            sequenceDiagram
                ...
                participant UI as Working Patterns UI
                participant BFF as BFF
                participant SVC as shared-cap-svc

                User->>UI: Navigate to Working Patterns
            ...
            **Screen: Working Patterns — Default State**

            ```
            ┌─────────────────────────────────────────────────┐
            │  [Calendar Management] > Working Patterns              │  ← Breadcrumb
            ├─────────────────────────────────────────────────┤
            │  ┌─ Core Data ─────────────────────────────────┐│
            ...
            Fields are defined authoritatively in `domain-specs/shared_cap-spec.md` §3 (Domain Model) and §6 (REST API). This feature renders the fields relevant to working patterns.
            ...
            See companion file: `contracts/aui/F-SHARED-CAP-001-02.aui.yaml`
...
| CAP_ADMIN | Y | Y | Y | Y |
            ...
            **AC-1: Apply Day Shift**
            - **Given** a Production Planner is on the Working Patterns screen
            - **When** they apply the standard day shift pattern to a machine calendar
            - **Then** Events are generated from the RRULE for the effective date range.

            **AC-2: Change to 3-Shift**
            - **Given** a Plant Manager is on the Working Patterns screen
            - **When** they switch a resource from single to three-shift pattern
            - **Then** The old pattern is superseded and capacity buckets are rebuilt.
            ...
            |---|---|---|
            | `F-SHARED-CAP-001-02` | T1 Reference Data | Validation of codes |
            ...
            |---|---|---|---|
            | `ext.f_t2_002_02.customFields` | zone | Additional fields from T3 domains | Zone hidden |
