...
> - **Status:** DRAFT
> - **Service ID:** `auto-ntf-svc`
> - **Suite:** `auto`
> - **Domain:** `ntf`
> - **Base Package:** `io.openleap.auto.ntf`
> - **API Path:** `/api/auto/ntf/v1`
> - **Port:** 8416
> - **DB Schema:** `auto_notification`
> - **Tier:** T2 — Common
...
This document specifies the **Notification Service** (`auto-ntf-svc`) as a T2 Common capability. It defines the domain model, business rules, use cases, REST API, events, data model, security, and quality attributes for the `Notifications & Alerts` bounded context.
...
- OpenAPI: `contracts/http/auto/ntf/openapi.yaml`
- Event Schemas: `contracts/events/auto/ntf/*.schema.json`
...
| Base Package | `io.openleap.auto.ntf` |
| API Base Path | `/api/auto/ntf/v1` |
| Port | 8416 |
| Database Schema | `auto_notification` |
| Tier | T2 — Common |
| Repository | `openleap-io/io.openleap.auto.ntf` |
...
**Base Path:** `/api/auto/ntf/v1`
**Authentication:** OAuth2 / JWT (Bearer). **Authorization:** scopes `auto.ntf:read`, `auto.ntf:write`, `auto.ntf:admin`.
...
Full payloads in `contracts/http/auto/ntf/openapi.yaml`.
...
| Event | Routing Key |
|-------|------------|
| NotificationDelivered | `auto.ntf.notification.delivered` |
| NotificationRead | `auto.ntf.notification.read` |
| NotificationFailed | `auto.ntf.notification.failed` |
| NotificationPreferenceChanged | `auto.ntf.preference.changed` |
| NotificationChannelCreated | `auto.ntf.channel.created` |
| NotificationChannelDisabled | `auto.ntf.channel.disabled` |
...
For 60 days after the promotion PR merges, `auto-ntf-svc` SHALL also publish every outbound event under its legacy `crm.ntf.*` routing key so existing CRM consumers continue to work. Bridge SHALL be removed after grace period ends (see §13).

---

## 8. Data Model

**Storage:** PostgreSQL 16, Schema: `auto_notification`.
...
DDL details deferred to implementation repo (`io.openleap.auto.ntf`).
...
|-----------|-------------|
| `F-AUTO-NTF-001` | Notification Center (in-app) |
| `F-AUTO-NTF-002` | Notification Preferences |
| `F-AUTO-NTF-003` | Channel Configuration (admin) |
| `F-AUTO-NTF-004` | Template Management |
| `F-AUTO-NTF-005` | Digest & Quiet Hours |
...
- Every outbound event available for extension subscribers (`auto.ntf.ext.*` extension-event channel for tenant-custom notification hooks).
...
- **Routing keys:** new keys `auto.ntf.*`; old keys `crm.ntf.*` re-emitted by this service for a 60-day grace period.
- **API:** new path `/api/auto/ntf/v1`; legacy path `/api/crm/ntf/v1` proxied to new endpoints for 60 days (HTTP 308 redirect).
- **Data:** schema renamed `crm_notification` → `auto_notification` via Flyway migration; tenant boundaries unchanged.
- **Port:** stays at 8416 (no client-side change during migration).

### 13.2 Post-Migration

- All CRM-internal references to `crm-ntf-svc` refactored to `auto-ntf-svc` within 60 days.
...
See `T2_Common/_auto_suite.md` SS1 for the shared-business UBL.
