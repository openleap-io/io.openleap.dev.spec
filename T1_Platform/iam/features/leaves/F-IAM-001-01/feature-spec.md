...
# F-IAM-001-01 — Create Human Principal

> **Conceptual Stack Layer:** Platform-Feature
> **Space:** Platform
> **Owner:** Domain Engineering Team
> **Companion files:** `F-IAM-001-01.uvl`, `F-IAM-001-01.aui.yaml`
...
> - **Status:** DRAFT
> - **Feature ID:** `F-IAM-001-01`
> - **Suite:** `iam`
> - **Node type:** LEAF
> - **Companion UVL:** `uvl/leaves/F-IAM-001-01.uvl`
> - **Companion AUI:** `contracts/aui/F-IAM-001-01.aui.yaml`
...
This feature lets a IAM_PRINCIPAL_ADMIN register a new human principal (user account) so a real person can authenticate and access the platform.
...
### 1.1 The User Goal
Register a new human principal (user account) so a real person can authenticate and access the platform.

### 1.2 User Scenarios
**Scenario 1: Admin onboards team member**
> Fill form, submit, Keycloak sync, welcome email

**Scenario 2: HR API creates principal**
> POST returns principal_id for FK
...
```
┌──────────────────────────────────────────────────┐
│  ← Back            Create Human Principal     [Cancel]    │
├──────────────────────────────────────────────────┤
...
|---|---|---|:---:|---|
| Username * | `iam-principal-svc` | Text | Y | 3-64 chars, unique per tenant |
| Email * | `iam-principal-svc` | Email | Y | Valid, unique per tenant |
| First Name * | `iam-principal-svc` | Text | Y | Max 100 |
| Last Name * | `iam-principal-svc` | Text | Y | Max 100 |
| Display Name | `iam-principal-svc` | Text |  | Auto if blank |
| Locale | `iam-principal-svc` | Select |  | locale catalog |
| Timezone | `iam-principal-svc` | Select |  | IANA |
| Tenant * | `iam-principal-svc` | Typeahead | Y | Active tenant |
| Send Welcome Email | `iam-principal-svc` | Checkbox |  | Default: true |

### 3.2 Actions
| Action | Trigger | Precondition | Confirm | Role | Outcome |
|---|---|---|:---:|---|---|
| Submit | Button | Form valid | — | `IAM_PRINCIPAL_ADMIN` | Navigate + toast |
...
See companion file: `contracts/aui/F-IAM-001-01.aui.yaml`
...
| Submit | Always | Form valid | `IAM_PRINCIPAL_ADMIN` |

---

## 8. Acceptance Criteria
**AC-001:** Given role `IAM_PRINCIPAL_ADMIN`, when all required fields filled and submitted, then record created and toast shown.
